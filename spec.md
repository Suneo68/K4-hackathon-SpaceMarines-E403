...
- [Sản phẩm 1: Tính năng Auto-responder của các Discord Bot truyền thống (MEE6, Dyno)]:
  - flow: Admin (TA) cài đặt sẵn các cú pháp cứng (ví dụ: `!deadline`, `!slide`). Học viên gõ đúng cú pháp $\rightarrow$ Bot trả về text cố định.
...
## §4. Thiết kế
- Lát cắt MỘT CÂU (1 user · 1 việc · 1 quyết định AI · 1 kết quả): Học viên đặt câu hỏi ở kênh `#🙋-hỏi-đáp` $\rightarrow$ AI tự động tra cứu dữ liệu mới nhất từ các kênh thông báo/bài học $\rightarrow$ Trả lời chính xác ngay lập tức kèm Link trích dẫn dẫn thẳng tới tin nhắn gốc.
...
## §7. Kiểm thử
- Chiều chất lượng + định nghĩa kiểm chứng được: Đánh giá dựa trên Tỷ lệ trích xuất đúng ngữ cảnh (Context Relevance $\ge 85\%$), Tỷ lệ trích dẫn nguồn chuẩn (Citation Accuracy 100%) và Tỷ lệ chống ảo giác (Zero Hallucination Rate 100% đối với thông tin deadline/lịch trình).
...
- Quality bar (chốt từ 23:59, giữ nguyên sau đó): "Đạt khi $\ge 85\%$ qua bộ test cases, và $0\%$ vi phạm lỗi Hallucination ở các case Hậu quả thật (L4)."
...
| :--- | :--- |
| **Spec** | `Trần Văn Thi` |
| **Evidence** | `Nghiêm Quốc Huy + Vũ Thế Lực + Trần Văn Thi` |
| **Validation** | `Nghiêm Quốc Huy + Vũ Thế Lực` |
| **Prompt** | `Hoàng Tuấn Hưng + Ngô Văn Linh` |
| **Code** | `Ngô Văn Linh` |
| **Demo** | `Vũ Thế Lực` |
...
- Multi-prototype:
  - Phương án A: Bot chỉ trả lời keyword tĩnh + FAQ cố định.
  - Phương án B: Bot dùng Semantic Search (ChromaDB RAG) + Citation Source Link + OCR Vision.
  - Lý do chọn: Phương án B được chọn vì đáp ứng câu hỏi tự nhiên của học viên, tự động cập nhật kiến thức thời gian thực và loại bỏ hoàn toàn rủi ro bịa đặt thông tin.
